...
class WH:
    ...
    """ PP : WHEN """
    def when_check(self, node,parser):
        """ check if time related PP """
        if (node.label() == "PP"):
            node_ner = parser.ner(" ".join(node.leaves()))
    ...
    def when(self, binary_t,parser):
        acc = ""
        time = False
        for i in range(len(binary_t[0])):
            node = binary_t[0][i]
            if self.when_check(node,parser) and i == 0:
                acc += "when"
                time = True
            ...
            if (str(i.label())) == "VP":
                for j in i:
                    ...
                    if (str(j.label()) == "PP" and
                       ...
                       not(self.when_check(j,parser))):
                       return "VP"
        return None

    def PP_location_deletion(self, si, C,parser):
        acc = ""
        tree = Tree.fromstring(parser.parse(si))
        if C == "PP":
            for node in tree[0]:
    ...
    def where(self, si,parser):
        C = self.contain_loc(si,parser)
        if C:
            si_filtered = self.PP_location_deletion(si, C,parser)
            binary_q = Binary.main(si_filtered,parser)
    ...
    def main(self, binary, si, NE,parser):
        # binary is string rep of binary question
        if binary:
            binary_t = Tree.fromstring(parser.parse(binary))
            when = self.when(binary_t,parser)
            where = self.where(si,parser)
            why = Why.main(si,parser)
            what_who = What_Who.main(si, NE,parser)
            if when:
                print("	*** when : ", str(when))
                return when      
            if where:
